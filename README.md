...
A self-hosted, git-backed daily companion for knowledge management, task tracking, vacation planning, appointment tracking, runbooks, notes, links, snippets, mail/ticket templates, a Message of the Day, a Picture of the Day, an RSS Reader, and an EOL Tracker. All data is stored as plain files in git repositories — human-readable, version-controlled, and usable without the frontend.
...
| **Move between sections** | In bulk-select mode, choose a target section from the toolbar dropdown and click Move; all selected links moved in one commit |
...
| Feature | Details |
|---------|---------|
| **Daily message** | One message shown on the widget dashboard; rotates deterministically by date (same message all day) |
...
| Feature | Details |
|---------|---------|
| **Daily display** | One entry from the collection shown each day on the widget dashboard; selected deterministically so the same day always shows the same entry |
...
| Feature | Details |
|---------|---------|
| **Daily display** | One meme shown on the widget dashboard each day; same deterministic selection as PotD |
...
| Feature | Details |
|---------|---------|
| **Product search** | HTMX search against the [endoflife.date](https://endoflife.date) API; shows matching products with links to their cycle page |
| **Cycle browser** | Per-product table of all release cycles showing latest version, support status (active / soon / EOL) and EOL date |
| **Tracking** | Track individual product/cycle pairs; stored as `eol/{id}.yaml` in the data git repo |
| **Live enrichment** | Tracked entries enriched with live API data (latest version, status, EOL date) on every page load; Redis-cached for 24 h |
| **Notes** | Per-entry freetext notes editable inline via HTMX |
| **Gantt timeline** | Per-product Gantt chart with server-computed bar positions; support / security / extended phases colour-coded; red today-line and year markers |
| **History** | Changes appear in the History view; module visible in filter dropdown |
...
| **Widget Dashboard** | `/widgets` — customisable dashboard with drag-and-drop (Sortable.js); widgets for tasks, vacations, appointments, favorites, RSS, repositories, Redis cache, /tmp usage, app version, countdown (configurable date + label), and a full calendar (HTMX prev/next month navigation); masonry layout (no empty gaps between rows); stats widgets compact on desktop; Edit mode to add/remove/reorder; layout persisted in settings; all labels fully translated (EN/DE) |
...
| **Widget Dashboard as start page** | `GET /` redirects to `/widgets`; the widget dashboard is the application start page |
