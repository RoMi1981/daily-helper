...
- **Brand** — link to the widget dashboard (`/widgets`)
...
The **Widget Dashboard** (`/widgets`) is the start page. `GET /` redirects there. See the [Widget Dashboard](#widget-dashboard) section for details on available widgets and how to customise the layout.
...
Click **☆ Pin** on any entry to pin it. Pinned entries appear in a dedicated section at the top of the widget dashboard and are visually highlighted in category view. Click **★ Pinned** to unpin.
...
Click the **★ star** button on any task card to mark it as a favorite. Starred tasks appear in the **Favorites** widget on the widget dashboard, grouped by module.
...
2. Check one or more links.
3. Choose the target section from the dropdown that appears in the toolbar.
4. Click **Move** — the selected links are moved in a single git commit.
...
The **Message of the Day** module shows a rotating daily message at the top of the widget dashboard.

### Creating MOTDs

Go to **MOTD → New Message** and enter the text. Only active messages rotate on the widget dashboard; toggle any message active or inactive with the button in the list.
...
The MOTD widget appears at the top of the widget dashboard. Click **→** to manually advance to the next message. Advancing is saved in Redis and resets at midnight so the same message is shown all day by default.

---

## Picture of the Day

The **Picture of the Day** (PotD) module shows one entry from your collection each day on the widget dashboard.
...
Click **›** on the widget dashboard to advance to the next entry for today. The offset is saved in Redis and resets at midnight.

### Lightbox

Click any image thumbnail (in the list or the widget dashboard) to open it full-screen in a lightbox overlay. Close with **Escape**, the **×** button, or a click on the dark backdrop. PDFs open as before (no lightbox).
...
One meme is shown on the widget dashboard each day. The selection is deterministic — the same date always shows the same meme. Click **›** on the widget dashboard to skip to the next meme (offset resets at midnight).

### Lightbox

Click any image thumbnail (in the list or the widget dashboard) to open it full-screen. Close with **Escape**, the **×** button, or a click on the backdrop.
...
The **EOL Tracker** module (`/eol`) helps you track end-of-life dates for software products using the [endoflife.date](https://endoflife.date) public API.

### Searching and Adding Products

1. Go to **EOL Tracker** and click **Track Software**.
2. Type a product name (e.g. `python`, `ubuntu`, `nodejs`) — results load via HTMX.
3. Click the product to view all its release cycles with support status.
4. Click **Track** next to any cycle to add it to your tracked list.

### Tracked Entries

The tracked list shows each product/cycle with live-enriched status (Active / EOL Soon / EOL), latest version, and EOL date. Statuses are fetched from the API and cached in Redis for 24 h.

Use the **Notes** button to add freetext notes inline (saved via HTMX without a page reload).

### Timeline View

Click **Timeline** next to any product to see a Gantt chart of all its release cycles. Colour-coded bars show active support, security-only support, and extended support phases. A red vertical line marks today.

### Integration

- **History** — tracked and updated entries appear in the History view with module filter "EOL Tracker"
- **Global search** — `/search` searches product name, cycle, label, and notes
- **Operations** — copy or move EOL entries between repos

---

## Widget Dashboard

`/widgets` — a customisable dashboard showing live data from all modules.

### Opening and Editing

Click **Widgets** in the sidebar to open the dashboard. Click **Edit** (top-right) to enter edit mode:

- **Add widgets** — click any widget in the Add panel to enable it
- **Remove widgets** — click the **✕** in the widget header
- **Reorder widgets** — drag and drop within edit mode (Sortable.js)
- **Widget settings** — click the **⚙** icon on widgets that have configurable options (e.g. Countdown)

The layout is saved automatically to `settings.json`.
...
| Sprint | Active sprint — total work days, holidays, approved vacation, blocked appointments, available days (with % capacity), and remaining days from today |
| Vacation Balance | Used / remaining vacation days for the current year |
| Next Vacation | Next approved vacation entry |
| Upcoming Appointments | Next appointments from the calendar |
| Upcoming Events | Upcoming ICS events |
| Favorites | Starred items across all modules |
| Recent Activity | Latest git commits |
| RSS Feed | Latest articles from your RSS feeds |
| Runbooks | Quick access to runbooks |
| Quick Capture | Inline task/note creation |
| Bookmarks | Pinned links |
| PotD | Picture of the Day (with copy-to-clipboard) |
| Meme | Meme of the day (with copy-to-clipboard) |
| Redis Cache | Key count, hit rate, memory |
| /tmp Usage | Disk usage of `/tmp` with color-coded bar |
| App Version | Running version / commit hash |
| Repositories | Configured repos with CA / GPG / identity badges |
| Countdown | Configurable target date + label; shows days remaining |
| Calendar | Full month calendar with HTMX prev/next navigation; vacation and appointment overlays |
| Recent Notes | Latest notes |

### Countdown Widget Settings

Click the **⚙** icon on the Countdown widget header to open the settings popover. Set a **Label** (e.g. "Holiday") and a **Target date**. The widget shows the number of days remaining, "Today" on the day itself, and days elapsed after the date.
