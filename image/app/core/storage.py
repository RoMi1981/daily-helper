...
class GitStorage:
    ...
    def _parse_history_path(self, path: str, action: str) -> "dict | None":
        ...
        # eol/{id}.yaml
        if path.startswith("eol/") and path.endswith(".yaml") and len(parts) == 2:
            eol_id = parts[1][:-5]
            data = _read_yaml(path)
            title = f"{data.get('product', '')} {data.get('cycle', '')}".strip() if data else eol_id
            return {
                "action": action,
                "path": path,
                "module": "eol",
                "category": "EOL Tracker",
                "slug": eol_id,
                "repo_id": self.repo_id,
                "title": title or eol_id,
                "deleted": deleted,
                "url": "/eol",
            }
