...
async def global_search(request: Request, q: str = "", date_from: str = "", date_to: str = ""):
    ...
    if q_stripped and storage and _search_cache_key is not None:
        ...
        # EOL Tracker
        if modules.get("eol", True):
            try:
                from modules.eol.storage import EolStorage

                es = get_primary_store("eol", storage)
                if es:
                    hits = [
                        e
                        for e in EolStorage(es).list_entries()
                        if ql in e.get("product", "").lower()
                        or ql in e.get("cycle", "").lower()
                        or ql in e.get("label", "").lower()
                        or ql in e.get("notes", "").lower()
                    ]
                    if hits:
                        groups.append(
                            {
                                ...
                                "results": [
                                    {
                                        "title": e.get("label")
                                        or f"{e.get('product', '')} {e.get('cycle', '')}",
