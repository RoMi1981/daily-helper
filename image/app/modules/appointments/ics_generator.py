"""ICS (iCalendar) generation for appointment entries with configurable export profiles."""
...
_TRANSP = {"free": "TRANSPARENT", "oof": "OPAQUE", "busy": "OPAQUE"}
_BUSYSTATUS = {"free": "FREE", "oof": "OOF", "busy": "BUSY"}


def _all_days(start_str: str, end_str: str) -> list[date]:
    """Return all calendar days from start to end (inclusive)."""
    try:
...
def _resolve(template: str, entry: dict, days: list[date]) -> str:
    replacements: dict[str, str] = defaultdict(str, {
...
def _vevent_allday(day: date, uid_base: str, subject: str, body: str,
                   show_as: str, category: str,
                   recipients_required: list[str], recipients_optional: list[str],
                   no_online_meeting: bool, dtstamp: str) -> str:
    ...
    lines = [
        ...
        _fold(f"X-MICROSOFT-CDO-BUSYSTATUS:{_BUSYSTATUS.get(show_as, 'BUSY')}"),
        "CLASS:PUBLIC\r\n",
    ]
...
def _vevent_timed(day: date, start_time: str, end_time: str, uid_base: str,
                  subject: str, body: str, show_as: str, category: str,
                  recipients_required: list[str], recipients_optional: list[str],
                  no_online_meeting: bool, dtstamp: str) -> str:
    try:
...
def generate_ics(entry: dict, profile: dict) -> str:
    """Generate ICS file content for an appointment entry using an export profile."""
    show_as = profile.get("show_as", "busy")
    ...
    subject_tpl = profile.get("subject") or "{title} {start_date}–{end_date}"
    body_tpl = profile.get("body") or ""

    days = _all_days(entry.get("start_date", ""), entry.get("end_date", ""))
    subject = _resolve(subject_tpl, entry, days)
    body = _resolve(body_tpl, entry, days)
    uid_base = entry.get("id", "appointment")
    dtstamp = datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%SZ")

    lines = [
        "BEGIN:VCALENDAR\r\n",
        "VERSION:2.0\r\n",
        "PRODID:-//Daily Helper//Appointments//EN\r\n",
        "CALSCALE:GREGORIAN\r\n",
        "METHOD:PUBLISH\r\n",
    ]
    ...
    for day in days:
        if all_day:
            lines.append(_vevent_allday(day, uid_base, subject, body,
                                        show_as, category,
                                        recipients_required, recipients_optional,
                                        no_online_meeting, dtstamp))
        else:
            lines.append(_vevent_timed(day, start_time, end_time, uid_base,
                                       subject, body, show_as, category,
                                       recipients_required, recipients_optional,
                                       no_online_meeting, dtstamp))
