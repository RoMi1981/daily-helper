...
class TestQuickCaptureModal:
    ...
    def test_save_task_via_modal(self, page: Page, live_server):
        ...
        # Wait for modal to close — git push completes before server sends 303
        expect(page.locator(".qc-backdrop")).not_to_have_class("open", timeout=20000)

        # Task should appear in tasks list
        page.goto(f"{live_server}/tasks")
        page.wait_for_load_state("networkidle")
        page.wait_for_selector(f"text=QC Task {uid}", timeout=20000)
...
class TestHelpKeyboardShortcut:
    ...
    def test_question_mark_on_dashboard_does_nothing(self, page: Page, live_server):
        """Pressing ? on the dashboard (no active module) does not navigate."""
...
class TestSearchHighlighting:
    ...
    def test_search_snippet_contains_mark_element(self, page: Page, live_server):
        ...
        page.wait_for_selector(".search-snippet mark", timeout=30000)
