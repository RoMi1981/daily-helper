...
# EOL pages that render real API content (cycles table, timeline chart).
# These are the pages most likely to overflow — tested separately because
# they depend on the endoflife.date API being reachable.
EOL_API_PAGES = [
    ("/eol/product/python", "EOL Cycles table", "table"),
    ("/eol/timeline/python", "EOL Timeline chart", ".eol-timeline-wrap"),
]


@pytest.mark.parametrize("viewport", VIEWPORTS)
class TestEolResponsiveLayout:
    """Overflow checks for EOL detail pages that render multi-column API content."""

    @pytest.fixture(autouse=True)
    def set_viewport(self, page: Page, viewport: dict):
        page.set_viewport_size(viewport)

    @pytest.mark.parametrize("path,label,selector", EOL_API_PAGES)
    def test_no_horizontal_overflow(self, page: Page, live_server, viewport, path, label, selector):
        """No horizontal overflow on EOL API pages at this viewport."""
        page.goto(f"{live_server}{path}")
        page.wait_for_load_state("networkidle")
        if page.locator("h1").count() == 0:
            pytest.skip("Page returned no content (API may be unreachable)")
        _check_no_horizontal_overflow(page, path, viewport)

    @pytest.mark.parametrize("path,label,selector", EOL_API_PAGES)
    def test_key_element_visible(self, page: Page, live_server, viewport, path, label, selector):
        """Key element visible on EOL API pages at this viewport."""
        page.goto(f"{live_server}{path}")
        page.wait_for_load_state("networkidle")
        if page.locator("h1").count() == 0:
            pytest.skip("Page returned no content (API may be unreachable)")
