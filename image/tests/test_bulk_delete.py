...
def test_move_links_to_section_moves_files(tmp_path):
    from modules.links.storage import LinkStorage

    git = FakeGit(tmp_path)
    src = LinkStorage(git, "default")
    l1 = src.create_link({"title": "L1", "url": "https://a.com"})
    l2 = src.create_link({"title": "L2", "url": "https://b.com"})
    count = src.move_links_to_section([l1["id"]], "work")
    assert count == 1
    assert not (tmp_path / "links" / "default" / f"{l1['id']}.yaml").exists()
    assert (tmp_path / "links" / "work" / f"{l1['id']}.yaml").exists()
    assert src.get_link(l2["id"]) is not None


def test_move_links_to_section_empty_list(link_storage):
    link_storage.create_link({"title": "X", "url": "https://x.com"})
    count = link_storage.move_links_to_section([], "other")
    assert count == 0
    assert len(link_storage._git._committed) == 1  # only the create commit


def test_move_links_to_section_single_commit(tmp_path):
    from modules.links.storage import LinkStorage

    git = FakeGit(tmp_path)
    src = LinkStorage(git, "default")
    l1 = src.create_link({"title": "A", "url": "https://a.com"})
    l2 = src.create_link({"title": "B", "url": "https://b.com"})
    before = len(git._committed)
    src.move_links_to_section([l1["id"], l2["id"]], "archive")
