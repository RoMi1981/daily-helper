...
def test_save_ticket_template_maxlength(client):
    resp = client.post(
        "/settings/ticket-templates",
        data={"ticket_template_desc_maxlength": "80"},
        follow_redirects=False,
    )
    assert resp.status_code == 303
    assert "ticket-templates" in resp.headers["location"]


def test_save_ticket_template_maxlength_persisted(client):
    client.post(
        "/settings/ticket-templates",
        data={"ticket_template_desc_maxlength": "120"},
        follow_redirects=False,
    )
    from core import settings_store

    cfg = settings_store.load()
    assert cfg["ticket_template_desc_maxlength"] == 120


def test_save_ticket_template_maxlength_zero_allowed(client):
    client.post(
        "/settings/ticket-templates",
        data={"ticket_template_desc_maxlength": "0"},
        follow_redirects=False,
    )
    from core import settings_store

    cfg = settings_store.load()
    assert cfg["ticket_template_desc_maxlength"] == 0


def test_save_ticket_template_maxlength_negative_clamped(client):
    client.post(
        "/settings/ticket-templates",
        data={"ticket_template_desc_maxlength": "-10"},
        follow_redirects=False,
    )
    from core import settings_store
...
def test_settings_page_shows_ticket_template_maxlength_field(client):
    resp = client.get("/settings")
    assert resp.status_code == 200
    assert "ticket_template_desc_maxlength" in resp.text
