...
def test_countdown_vacation_tomorrow(client_with_storage):
    ...
    # Skip on weekends: count_work_days(today, today) = 0 → template shows 0, not "tomorrow"
    if date.today().weekday() >= 5:
        pytest.skip("Skipped on weekends — tomorrow countdown requires today to be a workday")
